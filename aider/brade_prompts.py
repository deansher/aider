...
def format_brade_messages(
    ...
    - Reference in responses using <task_context>
    ...
    static_context = wrap_xml("static_context", system_prompt)

    # Build project context
    project_parts = []
    if repo_map and repo_map.strip():
        project_parts.append(wrap_xml("repository_map", repo_map))
    if readonly_text_files:
        files_xml = format_file_section(readonly_text_files)
        project_parts.append(wrap_xml("readonly_files", files_xml))
    if editable_text_files:
        files_xml = format_file_section(editable_text_files)
        project_parts.append(wrap_xml("editable_files", files_xml))
    project_context = wrap_xml("project_context", "".join(project_parts) if project_parts else "\n")

    # Build environment context
    environment_context = wrap_xml("environment_context", platform_info if platform_info else "\n")

    # Build task context
    task_parts = []
    if task_instructions:
        task_parts.append(wrap_xml("instructions", task_instructions))
    if task_examples:
        task_parts.append(format_task_examples(task_examples))
    task_context = wrap_xml("task_context", "".join(task_parts) if task_parts else "\n")

    # Add guidance about using context
    context_guidance = """<!-- This material is generated by the Brade Application and is not seen by the user. -->

How to Use Context Sections:

1. Static Context (<static_context>):
   - Contains your core capabilities and role
   - Reference when explaining your approach
   - Use to justify decisions

2. Project Context (<project_context>):
   - Shows current state of the codebase
   - Latest file contents supersede chat history
   - Use to understand project structure

3. Environment Context (<environment_context>):
   - Platform and runtime details
   - Use when suggesting commands or platform-specific code

4. Task Context (<task_context>):
   - Current task requirements and examples
   - Reference to stay focused on immediate goals
...
superceding any other file content shown in chat messages."""
    ...
    context_str = wrap_xml(
        "context",
        f"{context_guidance}\n{static_context}{project_context}{environment_context}{task_context}"
    )
